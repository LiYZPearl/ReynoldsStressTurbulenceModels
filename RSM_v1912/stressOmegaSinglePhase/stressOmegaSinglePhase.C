...
#include "stressOmegaSinglePhase.H"
#include "fvOptions.H"
#include "bound.H"
...
void stressOmegaSinglePhase<BasicTurbulenceModel>::correctNut()
{
    this->nut_ = k_/omega_;
...
template<class BasicTurbulenceModel>
stressOmegaSinglePhase<BasicTurbulenceModel>::stressOmegaSinglePhase
(
...
template<class BasicTurbulenceModel>
bool stressOmegaSinglePhase<BasicTurbulenceModel>::read()
{
...
template<class BasicTurbulenceModel>
void stressOmegaSinglePhase<BasicTurbulenceModel>::correct()
{
    if (!this->turbulence_)
    {
        return;
    ...
    (
     ...
     // - fvm::SuSp(((2.0/3.0)*gamma_)*alpha*rho*divU, omega_)
      - fvm::Sp(beta_*alpha*rho*omega_, omega_) //Dissipation
      - fvm::SuSp(-sigmad*rho/omega_*CD0/omega_,omega_) // Cross diffusion
    ...
    (
      ...
      - fvm::laplacian(alpha*rho*DkEff(), R)
      + fvm::Sp(C1_*alpha*rho*betaStar_*omega_, R)
      ==
        alpha*rho*P
      - (2.0/3.0*(1 - C1_)*I)*alpha*rho*betaStar_*omega_*k_
      - alphaHat_*alpha*rho*dev(P)
      - betaHat_*alpha*rho*(D-1.0/3.0*tr(P)*I) // Mean strain effect on pressure-strain correlation term
      - gammaHat_*alpha*rho*k_*dev(S) // Mean strain effect on pressure-strain correlation term
